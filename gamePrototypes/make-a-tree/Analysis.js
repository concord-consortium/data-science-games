    ...
    this.codapPhone.call(
        ...
        function ( iResult ) {
            if (iResult.success) {
                ...
                console.log("case count result : " + JSON.stringify(iResult) + " type:" + typeof tCaseCount);
                this.cases = [];
                this.expectedCaseCount = tCaseCount;

                //  now get each case by index
...
Analysis.prototype.gotOneCase = function( iResult ) {
    if (iResult.success) {
        ...
        if (this.cases.length >= this.expectedCaseCount) {
            this.host.gotCases( );
        }
